...
%pred_lbls = knnclassify(Xtest_pca', Xtrain_pca', train_lbls);
...
mean_vecs = zeros(size(Xtrain_pca,1),nClasses);
for cc=1:nClasses
    curr_ind = find(train_lbls==uniqueClasses(cc));
    Xc = Xtrain_pca(:,curr_ind);
    mean_vecs(:,cc) = mean(Xc,2);
end

Dmat = zeros(nClasses,size(Xtest_pca,2));
for ii=1:size(Xtest_pca,2)
    for cc=1:nClasses
        diff_vec = mean_vecs(:,cc) - Xtest_pca(:,ii);
