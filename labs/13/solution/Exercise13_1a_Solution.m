...
for i=1:length(contents)
    ...
    nSuperPixels = 300;  % change this variable to 
    ...
    V = [LMean'; AMean'; BMean'];
    Dmat = zeros(max_label);
    for ii=1:max_label
        for jj=1:max_label
            diff = V(:,ii) - V(:,jj);
            Dmat(ii,jj) = diff' * diff;
        end
    end
    sigma = mean(Dmat(:));
    Amat = exp(-Dmat/(sigma^2));
    ...
    %Save the result
    imwrite(SalMapFinal,[path_OUT, name, '_nSP',num2str(nSuperPixels),'_HeatKernel_PSE.png']);
    imshow(SalMapFinal)
