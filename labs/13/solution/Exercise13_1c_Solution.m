...
for i=1:length(contents)
    ...
    for ss = 1:length(nSuperPixelsVec)
        ...
        % Exercise: Use the DistFind() function included in PSE_pack
        % to calculate the affinity matrix
        Amat = DistFind(LMean,AMean,BMean,max_label);
    ...
    %Save the result
    imwrite(SalMapFinal,[path_OUT, name, '_multiResolution_PSE.png']);
    imshow(SalMapFinal)
