...
while not(done)
    ...
    for i = 1:numSeed
        ...
        if not(terminateTrack(i))
            ...
            % Task 1:
            % Calculate distance from seed point (Zref_vec) to all pixels (Z).
            % (use Euclidian distance in feature space)
            Zref = repmat(reshape(Zref_vec,[1 1 5]),[size(X) 1]);
            dist = sqrt(sum((Z-Zref).^2,3));
            % dist = sqrt(sum(bsxfun(@minus,Z,reshape(Zref_vec,[1 1 5])).^2,3)); % Faster way, using bsxfun
            % dist = reshape(pdist2(Zref_vec,reshape(Z,[],5)),size(Z,1),size(Z,2)); % Even faster way, using pdist2
