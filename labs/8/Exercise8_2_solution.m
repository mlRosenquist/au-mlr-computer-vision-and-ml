...
for ll=1:length(Lvec)
    for cc=1:length(Cvec)
        ...
        Gtrain = sqrt(2/L) * cos(W*train_data + repmat(bias,1,size(train_data,2)));
    
        % train the classifier using the training data
        A = ((Gtrain*Gtrain' + 1/C*eye(L))\Gtrain) * T';


        %%% test phase
        % calculate the mapped test data
        Gtest = sqrt(2/L) * cos(W*test_data + repmat(bias,1,size(test_data,2)));
...
for ll=1:length(Lvec)
    for cc=1:length(Cvec)
        ...
        Dtrain = distance_matrix(train_data, ref_data);  sigma = mean(mean(Dtrain));
        Htrain = exp(-Dtrain/(2*sigma));
